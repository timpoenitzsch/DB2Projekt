...
template<class T>
void fill_column(std::shared_ptr<ColumnBaseTyped<T>> col, std::vector<T> &reference_data)
{
...
template<class T>
bool equals(std::vector<T> reference_data, std::shared_ptr<ColumnBaseTyped<T>> col)
{
    for (unsigned int i = 0; i < reference_data.size(); i++)
    {
        T col_value = (*col)[i];
...
template<class T>
bool test_column(std::shared_ptr<ColumnBaseTyped<T>> col,
                 std::shared_ptr<ColumnBaseTyped<T>> col_new,
                 std::vector<T> &reference_data)
{
    ...
    bool result =
        *(std::static_pointer_cast<ColumnBaseTyped<T>>(copy)) == *(std::static_pointer_cast<ColumnBaseTyped<T>>(col));
    if (!result)
    {
        std::cerr << std::endl << "VIRTUAL COPY CONSTRUCTOR TEST FAILED!" << std::endl;
        return false;
    ...
    /****** STORE AND LOAD TEST ******/
    {
        ...
        // std::shared_ptr<Column<int> > col2 (new Column<int>("int column",INT));
        col_new->load("data/");
...
bool unittest()
{
    ...
    std::shared_ptr<ColumnType<ValueType>> col_one(
        new ColumnType<ValueType>(getAttributeString<ValueType>(), getAttributeType<ValueType>()));
    std::shared_ptr<ColumnType<ValueType>> col_two(
        new ColumnType<ValueType>(getAttributeString<ValueType>(), getAttributeType<ValueType>()));
